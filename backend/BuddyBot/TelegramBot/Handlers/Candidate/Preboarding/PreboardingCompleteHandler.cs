...
public class PreboardingCompleteHandler( UserService userService, CandidateService candidateService ) : IStepHandler
{
    public StepKind Step => StepKind.PreboardingComplete;
    ...
    public async Task HandleAsync( CallbackQuery callbackQuery, ITelegramBotClient botClient, CancellationToken cancellationToken )
    {
        ...
        {
            await BotMessageHelper.SendErrorMessage( botClient, callbackQuery.Message.Chat.Id, ErrorMessages.CandidateNotFound, cancellationToken, candidate );
            return;
        }
        ...
        await botClient.SendMessage(
            ...
            text: PreboardingMessages.PreboardingComplete,
            replyMarkup: Inline.PreboardingComplete(),
