...
public class ContactBonusesHandler : ICallbackHandler
{
    public bool CanHandle( string callbackData ) => callbackData == "ContactBonuses";
    ...
    public async Task HandleAsync( CallbackQuery callbackQuery, ITelegramBotClient botClient, CancellationToken cancellationToken )
    {
        ...
        string imagePath = Path.Combine( AppContext.BaseDirectory, "Static", "Images", "Мочалова Лена.jpg" );
        string fullName = "Елена Мочалова";
        string teamsUrl = "https://teams.microsoft.com/l/chat/0/0?users=elena.mochalova@travelline.ru";

        await using ( var stream = File.OpenRead( imagePath ) )
        {
            await botClient.SendPhoto(
                chatId: callbackQuery.Message.Chat.Id,
                photo: new InputFileStream( stream, "Мочалова Лена.jpg" ),
