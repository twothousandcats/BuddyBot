    ...
    public FeedbackMappingProfile()
    {
        CreateMap<Feedback, FeedbackListDto>()
            ...
            .ForMember( dest => dest.DepartmentName,
                opt => opt.MapFrom( src =>
                    src.Candidate != null && src.Candidate.Team != null && src.Candidate.Team.Department != null
            ...
            .ForMember( dest => dest.HRNames,
                opt => opt.MapFrom( src => src.Candidate != null && src.Candidate.HRs != null
                    ? src.Candidate.HRs.Select( hr => $"{hr.ContactInfo!.FirstName} {hr.ContactInfo.LastName}" )
                        .ToList()
                    : new List<string>() ) )
            .ForMember( dest => dest.MentorNames,
                opt => opt.MapFrom( src => src.Candidate != null && src.Candidate.Mentors != null
                    ? src.Candidate.Mentors.Select( m => $"{m.ContactInfo!.FirstName} {m.ContactInfo.LastName}" )
                        .ToList()
                    : new List<string>() ) )
